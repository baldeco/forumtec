function validar(){
	...
	const titulo = form.f_titulo;
	const palestrantes = document.getElementsByName("f_palestrantes[]");
	...
	for(let i=0; i < palestrantes.length; i++){
		checkboxes.push(palestrantes[i].checked);
	}
	...
	if(!checkboxes.some(verificaCheckboxes)){
		...
		palestrantes[0].focus();
		alert("Selecione um ou mais palestrantes");
		return false;
	}
